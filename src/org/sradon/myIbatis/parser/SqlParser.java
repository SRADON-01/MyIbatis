package org.sradon.myIbatis.parser;
import org.sradon.myIbatis.anno.Param;
import org.sradon.myIbatis.utils.ReflectUtils;
...
public class SqlParser {
    ...
    public static String getTableName(String sql) {
        sql = sql.toUpperCase();
        // 插入语句 INSERT INTO tb_xx(name
        if (sql.contains("INSERT INTO ")) {
            if (sql.contains(" ("))
                return sql.substring(sql.indexOf("INTO ") + 5, sql.indexOf(" ("));
            if (sql.contains("("))
                return sql.substring(sql.indexOf("INTO ") + 5, sql.indexOf("("));
            throw new RuntimeException("[ERR] 不支持的SQL语句: " + sql);
        }
        // 更新语句 UPDATE tb_xx SET name =
        else if (sql.contains("UPDATE ")) {
            if (sql.contains(" SET"))
                return sql.substring(sql.indexOf("UPDATE ") + 7, sql.indexOf(" SET"));
            throw new RuntimeException("[ERR] 不支持的SQL语句: " + sql);
        }
        // 删除语句 DELETE FROM tb_xx WHERE
        else if (sql.contains("DELETE FROM "))
            return sql.substring(sql.indexOf("FROM ") + 5, sql.indexOf(" WHERE"));
        else
            throw new RuntimeException("[ERR] 不支持的SQL语句: " + sql);
    }
